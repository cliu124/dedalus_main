...
class flag(object):
    ...
    def governing_equation(self,domain,solver_in=0):
        ...
        if self.flow in ['IFSC_2D']:
            ...
            if self.problem =='IVP':
                ...
                if self.flux_T:
                    problem.add_equation('Pe_T*dt(T_hat)-w_hat-(dz(dz(T_hat))-kx*kx*T_hat-ky*ky*T_hat)=-Pe_T*Pe_T*w_hat*integ(conj(w_hat)*T_hat+w_hat*conj(T_hat))/Lz-Pe_T*(U_0*1j*kx*T_hat+w_hat*dz(T_0))-Pe_T*W_0*dz(T_hat)')
                else:
                ...
                while solver.ok:
                    ...
                    if solver.iteration % 1000 == 0:
                        ...
                        if self.flux_T:
                            ...
                            #logger.info('max|T|: {}'.format(flow_out.max('T')))
                            logger.info('Nu: {}'.format(-1/dy_T_mean_q))
                            logger.info('U_0(z=0.25): {}'.format(np.real(solver.state['U_0']['g'][round(self.Nz/4)])))
    ...
    def post_store_after_run(self,solver):
        ...
        #Update 2022/10/07, save this checkpoint after run for restart
