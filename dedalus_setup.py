...
class flag(object):
    ...
    def governing_equation(self,domain):
        ...
        if self.flow in ['IFSC_2D']:
            ...
            if self.problem =='BVP':
                ...
                #coupling between real and imag due to shear
                if self.F_sin=='z':
                    problem.add_equation('dz(d_T_hat)-w_hat*dy_T_mean-(kx*kx+ky*ky)*T_hat-1j*Pe_T*kx*(z-1/2)*T_hat-eig_val*T_hat=0')
                    problem.add_equation('dz(d_S_hat)-1/tau*w_hat*dy_S_mean-(kx*kx+ky*ky)*S_hat-1j*Pe_S/tau*kx*(z-1/2)*S_hat-eig_val*S_hat=0')   
                else:
                    print(self.F_sin)
                    problem.add_equation('dz(d_T_hat)-w_hat*dy_T_mean-(kx*kx+ky*ky)*T_hat-1j*Pe_T*kx*F_sin*sin(ks*z)*T_hat-eig_val*T_hat=0')
                    problem.add_equation('dz(d_S_hat)-1/tau*w_hat*dy_S_mean-(kx*kx+ky*ky)*S_hat-1j*Pe_S/tau*kx*F_sin*sin(ks*z)*S_hat-eig_val*S_hat=0')   
    ...
    def initial_condition(self,domain,solver):
        ...
        if not pathlib.Path('restart.h5').exists() and self.problem != 'EVP':
