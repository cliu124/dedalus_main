...
if plot_config.visible==1 || plot_config.print==1 || plot_config.post==1
    ...
    % T_hat=p.u(p.np+1:2*p.np);
    % T0=p.u(2*p.np+1:3*p.np);
%     obj.u_tilde=u(1:p.np);
%     obj.v_tilde=u(p.np+1:2*p.np);
    obj.w_hat=u(1:p.np);
%     obj.p_hat=u(3*p.np+1:4*p.np);
    obj.T_hat=u(p.np+1:2*p.np);
    obj.S_hat=u(2*p.np+1:3*p.np);
    obj.T_0=u(3*p.np+1:4*p.np);
    obj.S_0=u(4*p.np+1:5*p.np);
...
%     end
    ...
    obj.u_tilde=p.mat.D1*obj.w_hat/(obj.kx+obj.ky);
...
%     plot_config.fontsize=20;
    ...
    p.my.Nu=p.mat.D1_full*[0;obj.T_0(:,1);0]+obj.dy_T_mean;
    clear data
    ...
    plot_line(data,plot_config);
    %for computing the Nusselt number, use the full matrix. 
    p.my.Nu_S=p.mat.D1_full*[0;obj.S_0(:,1);0]+obj.dy_S_mean;
