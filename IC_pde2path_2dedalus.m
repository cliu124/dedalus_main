...
for branch_ind=1:length(branch_name_list)
    ...
    for pt_ind=1:length(node_name.pt_list)
        ...
        if any(abs(point_list-point)<0.01)
            ...
            mkdir([IC_write_folder_name,num2str(point_list(ind))]);
            branch_name=branch_name_list{branch_ind};
            ...
            kx_final=obj_pde2path{branch_ind,ind}.kx;
            kx=kx_final*sqrt(2);
...
%             Lx2d=1;
            ...
            obj_dedalus{branch_ind,ind}.u(:,:,end)=2*real(1i*obj_dedalus{branch_ind,ind}.u_tilde*exp(1i*kx*x));
            obj_dedalus{branch_ind,ind}.d_u(:,:,end)=2*real(1i*obj_dedalus{branch_ind,ind}.d_u_tilde*exp(1i*kx*x));
            obj_dedalus{branch_ind,ind}.w(:,:,end)=2*real(obj_dedalus{branch_ind,ind}.w_hat*exp(1i*kx*x));
            obj_dedalus{branch_ind,ind}.d_w(:,:,end)=2*real(obj_dedalus{branch_ind,ind}.d_w_hat*exp(1i*kx*x));
            obj_dedalus{branch_ind,ind}.p(:,:,end)=2*real(obj_dedalus{branch_ind,ind}.p_hat*exp(1i*kx*x));
            obj_dedalus{branch_ind,ind}.T(:,:,end)=obj_dedalus{branch_ind,ind}.T_0*ones(size(x))+2*real(obj_dedalus{branch_ind,ind}.T_hat*exp(1i*kx*x));
            obj_dedalus{branch_ind,ind}.S(:,:,end)=obj_dedalus{branch_ind,ind}.S_0*ones(size(x))+2*real(obj_dedalus{branch_ind,ind}.S_hat*exp(1i*kx*x));
            obj_dedalus{branch_ind,ind}.d_T(:,:,end)=obj_dedalus{branch_ind,ind}.d_T_0*ones(size(x))+2*real(obj_dedalus{branch_ind,ind}.d_T_hat*exp(1i*kx*x));
            obj_dedalus{branch_ind,ind}.d_S(:,:,end)=obj_dedalus{branch_ind,ind}.d_S_0*ones(size(x))+2*real(obj_dedalus{branch_ind,ind}.d_S_hat*exp(1i*kx*x));
            ...
            h5_name_destination=[IC_write_folder_name,num2str(point_list(ind)),'/'...
