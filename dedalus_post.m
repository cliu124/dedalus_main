...
%                 data{3}.y=linspace(min(data{1}.y),10);
                ...
                plot_config.legend_list={1,['$\int E_',variable_name,'(k_x,k_z)dk_z$'],['$\int E_',variable_name,'(k_x,k_z)d k_x$'],'$2k_s/k_{opt}$'};
...
%             data{1}.y=obj.z_list/(2*pi/obj.k_opt);
            switch variable_name
                case {'T','S'}
                    data{1}.x=obj.z_list;
                    data{2}.x=squeeze(mean(mean(variable_data,2),3))+obj.z_list;
                case 'u'
