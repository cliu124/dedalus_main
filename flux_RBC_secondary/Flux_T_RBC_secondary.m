        ...
        for Ra_T_q_ind=1:length(Ra_T_q_list)
            ...
            for Pr_ind=1:length(Pr_list)
                ...
                %         A=[];% u
                        ...
                        [growth_rate(Lx_ind,Ra_T_q_ind,Pr_ind,ky_ind,kz_ind),max_ind]=max(real(eig_val));
                        eig_val_max{Lx_ind,Ra_T_q_ind,Pr_ind,ky_ind,kz_ind}=eig_val(max_ind);
                        eig_vec_max{Lx_ind,Ra_T_q_ind,Pr_ind,ky_ind,kz_ind}=eig_vec(:,max_ind);
                        ...
                        if flag.no_flux_com
                            ...
                            eig_val=diag(eig_val);
                            eig_val(find(real(eig_val)==Inf))=-Inf;
                            [growth_rate_no_flux_com(Lx_ind,Ra_T_q_ind,Pr_ind,ky_ind,kz_ind),max_ind]=max(real(eig_val));
                        ...
                        if flag.no_shear_com
                            ...
                            eig_val=diag(eig_val);
                            eig_val(find(real(eig_val)==Inf))=-Inf;
                            [growth_rate_no_shear_com(Lx_ind,Ra_T_q_ind,Pr_ind,ky_ind,kz_ind),max_ind]=max(real(eig_val));

                        end

                    end
                    [~,kz_max_ind]=max(squeeze(growth_rate(Lx_ind,Ra_T_q_ind,Pr_ind,ky_ind,:)));
                    kz_max(Lx_ind,Ra_T_q_ind,Pr_ind,ky_ind)=kz_list(kz_max_ind);
                    [~,kz_0_ind]=min(squeeze(abs(growth_rate(Lx_ind,Ra_T_q_ind,Pr_ind,ky_ind,3:end))));%This will get rid of the first two points that always gives growth rate close to zero 
                    kz_0(Lx_ind,Ra_T_q_ind,Pr_ind,ky_ind)=kz_list(kz_0_ind);
                end
            end
        end
    end
...
switch flag.name
    ...
    case 'Lx'
        ...
        plot_line(data,plot_config);
    case 'Ra_T_q'
        data{1}.x=Ra_T_q_list;
        data{1}.y=squeeze(kz_0);
        data{2}.x=Ra_T_q_list;
