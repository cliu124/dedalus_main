...
plot_config.legend_list={1,'DNS','$T_p=-0.0448\;{\rm ln}(Ra_{T,q}^{(g)}-Ra_{T,q})+0.6118$'};
plot_config.label_list={1,'$-{\rm ln}(Ra_{T,q}^{(g)}-Ra_{T,q})$','$T_p$'};
...
%plot_config.xlim_list=[1,4.676108e4,Ra_g];
...
error('1');
%Backup code to generate the old figure
...
% error('1');
% x=lsqr(-log(Ra_g-data{1}.x(fit_ind)),data{1}.y(fit_ind));
data{2}.x=-log(Ra_g-data{1}.x(fit_ind));
%old, that from fitting
