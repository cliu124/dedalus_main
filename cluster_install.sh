...
HDF5_VERSION=$HDF5_VERSION_MAJOR".2" 
...
#	export GETFILE="wget -nv --no-check-certificate"
	...
	echo "and a module list like the following (versions omitted):"
	echo "Currently Loaded Modulefiles:"
        echo "  	1) mpi-sgi/mpt.version     2) mpi-sgi/mpt             3) comp-intel/version   4) pkgsrc/version"
	echo
	echo "Here are your module files:"
	module list
	echo
...
EOF
    ...
    PYCONF_ARGS=$PYCONF_ARGS" --with-cxx-main="$MPICXX" --with-gcc="$MPICC" --with-system-ffi --enable-shared"  #--enable-optimizations" #  --with-system-ffi"
    PY_LDFLAGS="-lpthread"
...
export PYTHONPATH=${DEST_DIR}/lib/python$PYTHON_VERSION_SHORT/site-packages/:$PYTHONPATH
...
#critical if building against MKL, fixes MKL/FFTW symbol shadowing problem
...
[ALL]
extra_compile_args = $ARCH_CONF $MAKE_PROCS
...
#(${DEST_DIR}/bin/pip3 install git+http://bpbrown@github.com/bpbrown/numpy \
(${DEST_DIR}/bin/pip3 install numpy --no-binary :all: --progress-bar pretty \
    --global-option="config"     --global-option="--compiler=intelem" \
    --global-option="build_clib" --global-option="--compiler=intelem" \
    --global-option="build_ext"  --global-option="--compiler=intelem" 2>&1 ) 1>> ${LOG_FILE} || do_exit 

#( ${DEST_DIR}/bin/pip3 install numpy 2>&1 ) 1>> ${LOG_FILE} || do_exit

echo "Executing numpy speed test"
if [ ! -e ./numpy_speed_test.py ]
then
...
setup = "import numpy; x = numpy.random.random((4096,4096))"
count = 10
...
( ${DEST_DIR}/bin/pip3 install scipy --no-binary :all: --progress-bar pretty 2>&1 ) 1>> ${LOG_FILE} || do_exit
